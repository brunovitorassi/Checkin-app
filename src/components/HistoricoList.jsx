...
function FotoModal({ url, onClose }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.92)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:3000, padding:20 }} onClick={onClose}>
      <img src={url} alt="Foto do check-in" style={{ maxWidth:"100%", maxHeight:"90vh", borderRadius:12, objectFit:"contain" }} onClick={e=>e.stopPropagation()} />
    </div>
  );
}
      ...
      <>
        <div style={{ display:"flex", flexDirection:"column" }}>
          {checkins.map((c, i) => {
            ...
            return (
              <div key={c.id} style={{ background:"#0d1f35", border:"1px solid #1a2d4a", borderRadius:14, padding:"14px 16px", marginBottom:10, position:"relative" }}>

                {/* Foto + delete — absolute top right */}
                <div style={{ position:"absolute", top:14, right:14, display:"flex", gap:8, alignItems:"center" }}>
                  {c.foto_url && (
                    <img src={c.foto_url} alt="foto" style={{ width:40, height:40, borderRadius:8, objectFit:"cover", cursor:"pointer" }} onClick={()=>setFotoAberta(c.foto_url)} />
                  )}
                  {isAdmin && (
                    <button onClick={()=>onDelete(c.id)} style={{ background:"rgba(239,68,68,0.1)", border:"none", borderRadius:6, padding:"4px 8px", cursor:"pointer", color:"#ef4444", fontSize:12 }}>✕</button>
                  )}
                </div>

                {/* Linha 1: index + data + loja */}
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8, paddingRight: c.foto_url || isAdmin ? 60 : 0 }}>
                  <span style={{ fontSize:11, color:"#4a6080", fontWeight:700 }}>#{checkins.length - i}</span>
                  <span style={{ fontSize:12, color:"#38bdf8" }}>{formatDate(c.timestamp)}</span>
                  {c.loja && <span style={{ ...S.tag("purple"), fontSize:10 }}>🏪 {c.loja}</span>}
                </div>

                {/* Linha 2: usuário (dashboard only) */}
                {isDashboard && c.usuario && (
                  <div style={{ fontSize:12, color:"#94a3b8", marginBottom:4 }}>👤 {c.usuario}</div>
                )}

                {/* Linha 3: cliente */}
                <div style={{ fontSize:14, fontWeight:700, color:"#fff", marginBottom:6 }}>
                  {c.codigo_cliente ? `${c.codigo_cliente} — ${c.nome_cliente || "—"}` : (c.nome_cliente || <span style={{ color:"#4a6080" }}>Sem cliente</span>)}
                </div>
                ...
                <div style={{ marginBottom:6 }}>
                  <span style={{
                    ...
                    background:`${est.color}18`, border:`1px solid ${est.color}40`,
                    ...
                    <span style={{ overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{resumirEndereco(c.endereco)}</span>
                  </span>
                </div>

                {/* Linha 5: resumo */}
                {c.resumo_visita && (
                  <div style={{ fontSize:12, color:"#94a3b8", lineHeight:1.5, borderTop:"1px solid #1a2d4a", paddingTop:8, marginTop:4 }} onClick={()=>setResumoAberto(c)}>
                    {c.resumo_visita.slice(0, 120)}{c.resumo_visita.length > 120 ? "..." : ""}
                  </div>
                )}

                {/* Linha 6: retorno supervisor */}
                {c.retorno_supervisor && (
                  <div style={{ fontSize:12, color:"#a78bfa", marginTop:6, borderTop:"1px solid #1a2d4a", paddingTop:6 }}>
                    💬 {c.retorno_supervisor}
                  </div>
                )}
              </div>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", minWidth: isDashboard ? 1100 : 820 }}>
          ...
          <tbody>
            {checkins.map((c, i) => {
              ...
              const rowBg = isLight ? (i%2===0 ? "#ffffff" : "#f8fafc") : (i%2===0 ? "#0a1628" : "#07101f");
              return (
